...
- **The treatment is its own artifact** (`piece.treatment`) with its own **stage** (`treatment_map`, which already existed in `lib/marketing/stages.ts` between Script and Record, marked "soon" — this fills it in). Per beat: `SCREEN:` the one bold idea, `TOUCH:` the interaction + transition. It is the brief handed to the animation build.
- **Both are generated in ONE pass** (`===SCRIPT===` / `===TREATMENT===` delimiters, split by `splitScriptAndTreatment`), so they are coherent by construction and **share beat labels** so they cannot drift. Redrafting the script regenerates the treatment with it.
