...
The ramp: at 1 narrative a week the typed kit gives LinkedIn 4 of 7 days and Instagram 5 of 7, while TikTok and YouTube get 3 of 7. We fill the second daily slot by adding narratives, never by padding kits.
...
|---|---|---|
| Narrative | `concept-store` doc | + `lane` (marrs / polynize), + `article` (the long form), + `gate` (1 to 5 / shipped), + `idea_ref` |
| Piece | `piece-store` (exists, close) | + `narrative_ref`, + `channel` (one platform each), + `master_ref` (which asset it carries), + `queued_slot`, + `metricool_id` |
...
|---|---|---|
| **1 · Idea** | Ideas inbox + store | Lane picker, "Develop" creates the Narrative |
...
| **Learn** | nothing | Metricool analytics pull, weekly; rolls up per narrative; winners flag as exemplars (exemplar store exists) |
...
**Week 1 · Spine + Ship + the pilot.** Narrative/lane/gate fields on the stores. Per-channel slots. The wave screen. Then the pilot: 3 Types pieces assembled by hand where gates don't exist yet, and the Metricool button pressed for real (the D18 gate, still never fired). *The week ends with published content.*
...
**Week 3 · Article + Idea + the board.** April's two-pane article screen. Lane picker on the inbox. The board (every narrative at its gate) becomes the marketing home.

**Week 4 · The two new capabilities.** Prezie frame export feeding carousel and quote-image cards. Mobile pass over every gate.

**Then: the hold.** Four narratives through the gates, one a week, nothing new built. The Learn loop is built *during* the hold (it needs published data to pull anyway).
...
4. **The board replaces the marketing home.** Streams page goes; every narrative at its gate is the home. Yes?
