...
> **Build status (2026-07-02).** T2, T3, T4, T6 shipped and deployed. T1 is running on an **interim store** (pieces persist in `content_shoot_sheets` keyed `marketing/{owner}/{pieceId}`, owner-scoped) because migration `0009` needs DB creds only Marrs has — swapping to `content_pieces` is a store-internal change, no route/UI churn. The **dashboard shell** (`/console/marketing`: streams-by-owner + in-development pieces) was pulled forward from "After Phase 1" per the PM steer and shipped alongside T3. **T5 is reframed** (D16): it is now the **intake screen** (April interviews in-console → concept doc), not just a draft round-trip. Its dependency-free groundwork is now **shipped on the interim stand-in**: the `lib/agents/` socket seam (`converse` + jobs), the interim OpenRouter "April" provider, the interim jobs store, the concept store (keyed on the eventual bucket path), the intake screen (`/console/marketing/intake`), the concept view (`/console/marketing/concept/[slug]`), and the Concept bank on the dashboard. So the interview → concept-doc flow is fully working and testable today. **The real April is LIVE (2026-07-07): `AGENT_PROVIDER=hermes` is set in production.** The interview runs console-side in April's name (her persona + brand-voice doc from the bucket, billed to `APRIL_OPENROUTER_API_KEY`); `concept_finalize` is claimed by the real April pull worker (`april-bridge.service` on Lightsail) via `/api/agents/jobs/claim` + `/complete` and its markdown is written by the console to the `polynize-agents` S3 bucket at `pam/concept-bank/{owner}/`. First end-to-end concept doc written and rendered on 2026-07-07. The interim OpenRouter stand-in is now dormant (the fallback if `AGENT_PROVIDER` is unset). Migration `0009` is still not applied — the jobs store rides `content_shoot_sheets` (interim); concept docs now live in S3. All read/write and chat paths are hardened against malformed rows and data loss (runtime shape guard + PUT validation + subtree error boundary; chat applies are editor-locked, one-click undoable, and route through a serialized last-write-wins autosave). T4's chat used the shared `lib/llm` OpenRouter layer; the LLM is currently the repo default (DeepSeek via OpenRouter), not yet a dedicated per-agent "April/Mikey" key.
...
**T5 — The intake screen + the April socket (reframed 2026-07, per D16 / PM note).** Originally "the April draft round-trip." Now: the **intake screen** at the top of the spine, where April **interviews the owner in-console** (SOC 2: no Slack) through the context-chat, and the interview produces `core-concept-{framing}.md` into `pam/concept-bank/{owner}/` + an index row. That concept doc then feeds the Script screen (which today assumes it already exists). The socket contract is specified in `agent-socket-contract.md` (sync `converse` for the interview; async `concept_finalize` / `script_draft` jobs). Interim = OpenRouter/DeepSeek stand-in behind the same seam; real April swaps in once provisioned.
*Done:* an owner starts a concept on the intake screen, is interviewed to a concept doc, and it flows into the Script screen — through the socket, validated by one real implementation.
*Dependency-free groundwork buildable now (April not required):* the socket contract (done — `agent-socket-contract.md`), the `lib/agents/` provider seam + interim jobs store, and the intake screen shell backed by the interim stand-in. Gated on April: the real interview quality. Gated on bucket creds + owner-key: the concept-doc write to the bucket (interim store until then).
